...
SCg.Editor.prototype = {
    ...
    init: function(params)
    {
        ...
        this.typeProcedure = {
            ...
            //'scg-type-procedure-constr': SCgEditMode.SCgModalConstr
        };
    ...
    bindToolEvents: function() {
            ...
            input.keypress(function (e) {
                if (e.keyCode == KeyCode.Enter || e.keyCode == KeyCode.Escape) {
                    ...
                    if (e.keyCode == KeyCode.Enter){
                        user.changeIdtfCommand(self.scene.selected_objects[0],self.scene.selected_objects[0].text,input.val());
        ...
        cont.find('#scg-tool-change-type').click(function() {
            ...
            $(container + ' .popover .btn').click(function() {
                var obj = self.scene.selected_objects[0];
                user.changeTypeCommand(obj,obj.sc_type,self.typesMap[$(this).attr('id')]);
        ...
        cont.find('#scg-tool-delete').click(function() {
            user.deleteObjectCommand(self.scene.selected_objects[0]);
        ...
        //Undo operation
        cont.find('#scg-tool-undo').click(function(){
            // console.log("Undo Operation");
            var user = CommandManager.getUser();
            user.undo();
            self.onSelectionChanged();
        ...
        //Alignment operation
        cont.find('#scg-tool-alignment').click(function(){
            // console.log("Undo Operation");
            var user = CommandManager.getUser();
            user.gridAlignmentCommand();
            self.onSelectionChanged();

        });

        //Redo operation
        cont.find('#scg-tool-redo').click(function(){
    ...
    onSelectionChanged: function() {
        ...
        if(user.getCurrent() > 0){
            this._enableTool('#scg-tool-undo');
        }else {
            this._disableTool('#scg-tool-undo');
        }

        if(user.getCurrent() < user.getCurrentInRedo()){
            this._enableTool('#scg-tool-redo');
        }else {
            this._disableTool('#scg-tool-redo');
        }
    ...
    onModalChanged: function() {
        ...
        update_tool('#scg-tool-redo');
        update_tool('#scg-tool-undo');
    },
...
SCg.Scene = function(options) {
    ...
    this.boolDeleteAttributes = false;
    ...
    user = new CommandManager.User(this);
...
SCg.Scene.prototype = {
    ...
    onMouseMove: function(x, y) {
        ...
        if(this.edit_mode == SCgEditMode.SCgModeSelect || this.edit_mode == SCgEditMode.SCgModalConstr){
            if(this.scpObj.length != 0){
        ...
        if(this.scpObj != 0){
            for(var i=0;i<this.scpObj.length;i++){
                ...
                if(this.scpObj[i] instanceof SCg.ModelBus){
                    ...
                    this.scpObj[i].setPoints([{x: ScgObjectBuilder.points_scp_node_bus[0]+100, y: ScgObjectBuilder.points_scp_node_bus[1]}]);
                }
    ...
    deleteAttributesConstrMode: function(){
        ...
        user.deleteAttributesOfSCpTemplate(this.selected_attributes);
        this.updateObjectsVisual();
        this.updateRender();
    },
    ...
    onMouseDownObject: function(obj) {
        ...
        if (this.edit_mode == SCgEditMode.SCgModeSelect) {
            ...
            x_last = obj.position.x;
            y_lats = obj.position.y;
        }
    ...
    onMouseUpObject: function(obj) {
        ...
        if (this.edit_mode == SCgEditMode.SCgModeSelect) {
            ...
            if (obj == this.focused_object) {
                ...
                if(this.scpObj.length == 0){
                    user.moveCommand(obj,x_last,y_lats,this.mouse_pos.x,this.mouse_pos.y);
                }
                this.updateObjectsVisual();
            }
        ...
        if (this.edit_mode == SCgEditMode.SCgModalConstr){
            ...
            if (obj == this.focused_object && obj instanceof SCg.ModelEdge && obj.is_selected && !this.deleteAction) {
                ...
                user.paintSCpCommand(this.nodes_for_attribute,this.arcs_for_attribute,obj,this.mouse_pos.x,this.mouse_pos.y)
                this.updateObjectsVisual();
                this.updateRender();
            }
            this.focused_object = null;
